...
const handleNext = async () => {
  if (step < steps.length - 1) {
    ...
    const userData = {
      ...
      selectedSkills,
      selectedInterests,
      selectedPersonalityTypes: selectedPersonality,
      selectedFinancialLevels: selectedFinance,
    };

    try {
      const response = await axios.post(
        "http://localhost:8080/api/careers/recommend",
        userData,
        {
          headers: {
      ...
      localStorage.setItem("userProfile", JSON.stringify(userData)); // optional
      navigate("/dashboard");

    } catch (error) {
      console.error("Error submitting data:", error);
      alert("Failed to submit. Please ensure you're logged in.");
    }
  }
};
  const handlePrevious = () => {
